	...
	var DataCatalogView = Backbone.View.extend({
		...
		showResults: function (page) {
			...
			if(gmaps && (searchModel.get('north') != null)){ //If we have Google Maps enabled and a map filter is set
				...
				query += searchModel.getSubQuery("geohash");
			}
...
*/
		...
		*/
		...
		drawTiles: function(){
			...
			//Draw a world-wide tile to represent tiles with 0 datasets
			/*var worldBounds = this.map.getBounds(),
			    ...
			    worldTile = new google.maps.Rectangle(worldTileOptions);
			
			//Save it so we can remove it later
			//this.tiles.push({text: null, shape: worldTile});
			*/
